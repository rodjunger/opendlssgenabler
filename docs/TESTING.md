...
| Palworld (Steam) | Direct3D 12 | 310.4.0 | RTX 3080 | Its menu offers the multipliers; 2x, 3x and 4x confirmed at `presented` 2, 3 and 4, with 384 kernels supplied and none refused. The game defaults to Direct3D 11, where Streamline refuses frame generation, so it needs the `-dx12` launch option. Its executable does not import `version.dll`, so the proxy is installed as `dxgi.dll` |
